...
|---|---|---|---|---|---|
| 1 | Nutrición | 12.50% | Exactitud de fórmulas (BMR/TDEE en `calculations.ts`), rangos de macros (`macroValidation.ts`), objetivos clínicos (`clinicalTargets.ts`), micronutrientes, coherencia kcal↔macros de los planes generados | Contraste contra guías (EFSA, OMS, SENC, posicionamientos AND/Academy); tests unitarios como evidencia; muestreo de 20 planes generados recalculados a mano | `healthcare-reviewer` + Humano-DN |
| 2 | Seguridad alimentaria | 10.42% | 14 alérgenos UE, poblaciones de riesgo (embarazo, TCA, IMC extremo — `clinicalSafety.ts`), mínimos calóricos seguros, interacciones patología-dieta, avisos y disclaimers | Checklist de seguridad clínica por flujo; casos límite sintéticos (paciente embarazada, IMC<18.5, alergia múltiple) pasados por el generador | `healthcare-reviewer` + Humano-DN |
| 3 | Motor de recetas | 9.38% | Variedad, tasa de repetición entre planes, datos nutricionales por receta, cobertura por objetivo/patología/cultura, ICR medio (§5) | Inventario del corpus (FoodDatabase + salidas de Gemini); scoring ICR muestral; matriz de cobertura | IA-Orquestador (con WF-R) |
| 4 | IA | 8.33% | Calidad de prompts (`geminiService.ts`), tasa de JSON válido, guardrails, alucinaciones nutricionales, coste y latencia por generación | Batería de evals: ≥30 casos sintéticos de referencia con salida esperada; medición de tasa de error, latencia p95 y coste medio | IA-Orquestador |
| 5 | UX | 8.33% | Flujos críticos (crear paciente → generar plan → ajustar → exportar PDF → seguimiento), fricción (nº de clics), estados vacíos/error/carga, mensajes | Recorridos E2E instrumentados (`e2e-runner`); heurísticas de Nielsen; conteo de pasos por tarea | `e2e-runner` + Humano-PO |
| 6 | Personalización | 8.33% | Variables capturadas en `PatientForm` (objetivos, patologías, alergias, preferencias, presupuesto, horarios) y cuántas influyen realmente en el plan; adaptación al progreso real | Trazabilidad campo→prompt→plan: matriz de variables declaradas vs. usadas | IA-Orquestador + Humano-DN |
| 7 | Arquitectura | 7.29% | Acoplamiento (tamaño de `App.tsx` y `useAppData`), separación de capas, tipado estricto, duplicación, gestión de estado, manejo de errores | `tsc --noEmit`, métricas de tamaño/complejidad por fichero, revisión de dependencias entre módulos | `architect` + `typescript-reviewer` |
| 8 | Rendimiento | 6.25% | LCP, INP, CLS, tamaño de bundle, re-renders, tiempo de generación de plan | Lighthouse CI (móvil y desktop), `vite build` con análisis de bundle, React Profiler en flujos críticos | `performance-optimizer` |
| 9 | Escalabilidad | 5.21% | RLS y políticas Supabase, índices, consultas N+1, multi-tenant (varias nutricionistas), límites de localStorage vs. remoto, offline (sw.js) | Supabase advisors, revisión de `couples_diets.sql` y esquema, prueba de volumen sintética (500 pacientes) | `database-reviewer` |
| 10 | Accesibilidad | 5.21% | WCAG 2.2 AA: contraste, foco, teclado, etiquetas ARIA, lector de pantalla, tamaños táctiles | axe-core automatizado en todas las vistas + recorrido solo-teclado + zoom 200% | `e2e-runner` (audit a11y) |
| 11 | UI | 5.21% | Consistencia visual (tokens Tailwind), jerarquía, responsive (375/768/1280), modo oscuro, densidad de información clínica | Inspección sistemática por vista con capturas en 3 breakpoints × 2 temas; checklist de consistencia | IA-Orquestador + Humano-PO |
| 12 | Retención | 5.21% | Motivos de vuelta (recordatorios `useNotifications`, seguimiento, informes), fricción de re-uso, presencia de analítica para medir uso real | Inventario de mecanismos de retención existentes vs. matriz de referencia del sector; verificación de instrumentación | Growth (IA) + Humano-PO |
| 13 | Monetización | **0%** ⚠️ | Repesada a 0% en M12 de la iteración 001 (2026-07-09), con aprobación del Humano-PO: la app es herramienta interna mono-cliente, no producto comercial — perseguir un modelo de precios no aporta valor real. Si esa decisión de negocio cambia, se re-incluye con su peso original (4%) | — (excluida del cálculo) | — |
| 14 | Gamificación | 3.13% | Feedback de progreso (ProgressTracker), hitos, rachas de adherencia, refuerzo positivo clínicamente apropiado | Inventario vs. catálogo de mecánicas apropiadas para salud (sin dark patterns ni presión sobre TCA) | Growth (IA) + Humano-DN |
| 15 | Calidad del contenido | 3.13% | Textos de UI, tono clínico, ortografía ES, consistencia terminológica, calidad de PDFs generados (`pdfService`) | Revisión lingüística sistemática + muestreo de 10 PDFs generados | IA-Orquestador + Humano-DN |
| 16 | Competencia | 2.08% | Posición vs. Nutrium, Dietopro, i-Diet, Nutritics y apps consumer (MyFitnessPal, Yazio) | Matriz feature-gap actualizada por investigación web; identificación de diferenciales defendibles | Growth (IA) |
...
**Cambio de pesos (M12, iteración 001, 2026-07-09):** Monetización pasó de 4% a 0% (excluida del cálculo de la nota global) por decisión explícita del Humano-PO — la app sigue siendo una herramienta interna para una única clínica, no un producto a comercializar. El 4% liberado se redistribuyó proporcionalmente entre las 15 categorías restantes (factor ×100/96). Aplica desde la iteración 002 en adelante; el histórico de la iteración 001 conserva los pesos originales para no reescribir su cálculo ya cerrado.
...
- [ ] **4. Auditoría baseline:** ejecutar M1 completo una vez y registrar el `SCORECARD.md` inicial (iteración 000). **Pendiente — es la primera ejecución real del loop (M1 de la iteración 001).**
- [x] **5. Accesos y límites:** API key de Gemini y proyecto Supabase operativos; presupuesto por iteración definido (horas-IA, tokens, nº de mejoras). *(2026-07-09: presupuesto fijado — 8–16 h-IA / 5–8 mejoras por iteración, ver §3.6. `.env.local` presente en el repo (no se ha leído su contenido); operatividad real de las claves se confirmará en la primera llamada real de M1.)*
...
- [x] **9. Herramientas de medición instaladas:** Lighthouse, axe-core, análisis de bundle — verificadas con una ejecución de prueba. *(2026-07-09: `lighthouse@13.4.0` y `@axe-core/cli@4.12.1` instalados como devDependencies y verificados con `--version`. 0 vulnerabilidades en dependencias de producción tras la instalación — las 19 nuevas son transitivas de telemetría de Lighthouse (Sentry/OpenTelemetry), solo dev, registradas como O-007 en `BACKLOG.md`.)*
...
**El loop no arranca hasta que los 10 ítems estén marcados. Queda 1 pendiente: #4, la auditoría baseline — es el primer M1 real, con presupuesto de 3–5 h-IA según §1.2, y se ejecuta en la iteración 001.**
