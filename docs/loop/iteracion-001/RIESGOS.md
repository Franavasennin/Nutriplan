...
| R-04 | Datos (RGPD) | MEJORA-003 (RLS) toca infraestructura real de datos de salud de pacientes | Baja (si se sigue el proceso) / Alta si se aplica sin verificar | **Alta** | **No se aplica ningún cambio en el proyecto Supabase real sin aprobación explícita del Humano-PO.** Script de migración preparado y revisable, con rollback documentado, antes de cualquier ejecución real | Humano-PO (Fran) — aprobación obligatoria | **Bloqueante hasta aprobación — no se ejecuta la parte de aplicación sin ella** |
