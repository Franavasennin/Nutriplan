  ...
  return (
    <div className="min-h-screen bg-[#f6faf7] text-[#102216]">
      ...
      <main className="max-w-3xl mx-auto px-4 py-6 flex flex-col gap-6">
        ...
        <div className="no-print flex gap-2 overflow-x-auto pb-1">
          {plan.weeklyPlan.map(day => (
            <button
              ...
              className={`shrink-0 px-4 h-10 rounded-lg text-sm font-bold transition-all ${
        ...
        {/* Lista de la compra */}
        <div className="bg-white rounded-xl border border-gray-100 p-4">
          <p className="text-sm font-black mb-3">🛒 Lista de la compra</p>
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
            {coupleShoppingList.map(cat => (
              <div key={cat.category}>
                <p className="text-[10px] font-black uppercase text-gray-400 mb-1">{cat.category}</p>
                <ul className="flex flex-col gap-0.5">
                  {cat.items.map(item => (
                    <li key={item.name} className="text-[11px] text-gray-600 flex gap-1">
                      <span>□</span>{item.name}
                      {item.amounts.length > 0 && <span className="text-gray-400">({item.amounts.join('/')})</span>}
                    </li>
                  ))}
                </ul>
              </div>
